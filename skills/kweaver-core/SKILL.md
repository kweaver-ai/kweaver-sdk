...
| `context-loader` | MCP 分层检索 | `context-loader --help`, `context-loader <subcommand> --help`, `context-loader search-schema <kn-id> <query>`（也支持 `--kn-id <id>` flag；省略时回退到 deprecated 的 `context-loader config`） | `references/context-loader.md` |
