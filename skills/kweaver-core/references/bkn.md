...
kweaver bkn create-from-ds <ds_id> --name <name> [--tables <t1,t2>] [--build/--no-build] [--timeout 300]  # 自动创建 resource + 全列 mapped_field；与手动的 `object-type create` 不同，后者需自备 resource；CLI 会为 create 补全 mapped_field / 拉取 resource 字段，见 `references/build-kn-from-db.md`
...
kweaver bkn object-type create <kn_id> --name <n> --resource-id <res> --primary-key <pk> --display-key <dk> [--property '<json>' ...]  # --resource-id 接受 vega-backend resource ID；无 --property 时从 resource 拉全字段并生成 mapped_field；有 --property 时可省略 mapped_field（CLI 补全）
...
|---|---|---|---|
| `resource` | `kweaver resource list` 的 vega-backend 资源 ID | 通过 Vega 实时查询 | 不需要 / 不支持 |

`object-type create --resource-id` 的 `--resource-id` 参数传入 vega-backend 资源 ID（`kweaver resource list` 返回），CLI 自动设置 `data_source.type=resource`。
